    ...
    private function afficherNoteEtCommentaire($depot)
    {
        if (!empty($depot['note'])) {
            ...
            <div class="p-3 bg-light border rounded mb-3">
                ...
                <p><strong>Note :</strong> <?= htmlspecialchars($depot['note']) ?>
                    / <?= htmlspecialchars($depot['note_max']) ?></p>
                <p class="mb-0"><strong>Commentaire
                        :</strong> <?= htmlspecialchars($depot['commentaire']) ?: 'Aucun commentaire' ?></p>
            </div>
            <?php
        }
    }
        ...
        if ($depot['statut'] !== 'Remis') {
            ...
            <form action="index.php?module=depotetud&action=upload&idProjet=<?php echo $idSae; ?>" method="post" enctype="multipart/form-data" class="mt-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="file" class="form-control" id="fileUpload-<?= $depot['id_rendu'] ?>"
                           name="uploaded_files[]" multiple required>
                </div>
